  ...
  <el-dropdown trigger="click" @command="onCommand">
    ...
    <template #dropdown>
      <el-dropdown-menu>
        ...
        <el-dropdown-item v-if="props.comment.isSelf" divided command="remove">删除</el-dropdown-item>
      </el-dropdown-menu>
    </template>
  </el-dropdown>
</template>
...
import { ElDropdown, ElDropdownItem, ElDropdownMenu, UToast } from 'undraw-ui';
...
const onCommand = (command) => {
  switch (command) {
    ...
    case 'report':
      ...
      break;
    case 'copy':
      // 使用正则表达式删除“回复 <span...>@username:</span>”部分（包括任意用户名）
      const pattern = /回复 <span[^>]*>@[^<]+:<\/span>\s*/;
      const mainContent = props.comment.content.replace(pattern, '');
      copy(mainContent);
