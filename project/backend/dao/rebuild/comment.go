...
func RebuildCommentUserLikeOrHateMapping(userID, objID int64, objType int8, like bool) ([]int64, bool, error) {
	...
	// 需要移除在 niumahome:comment:rem:cid 中的 comment_id
	...
	if len(list) != len(exists) {
		return nil, false, errors.Wrap(niumahome.ErrInternal, "rebuild:RebuildCommentUserLikeOrHateMapping: list and exists' length not equal")
	}
