...
-- 参数定义
local keyCommentUserLikeOrHateIDs = KEYS[1] -- niumahome:comment:userlikeids:（记录某个主题下，一个用户给哪些评论点过赞）
local keyCommentRemCidSet = KEYS[2]         -- niumahome:comment:rem:cid（记录待删除的 cid_uid）
local keyCommentLikeOrHateCount = KEYS[3]   -- niumahome:comment:like:（记录点赞数）
