...
func CreatePost(doc *models.PostDoc) error {
	...
	_, err := clnt.Index("niumahome_post_index").
...
func GetPostDetail(PostID int64) (*models.PostDoc, error) {
	...
	resp, err := clnt.Get("niumahome_post_index", strconv.FormatInt(PostID, 10)).Do(ctx)
	if err != nil {
		return nil, errors.Wrap(err, "elasticsearch: get post failed")
	}
	if !resp.Found {
		return nil, niumahome.ErrNoSuchPost
	}
...
func getPostIDsHelper(query string, pageNum, pageSize int64) ([]string, int, error) {
	...
	res, err := lowlevelClnt.Search(
		lowlevelClnt.Search.WithIndex("niumahome_post_index"),
	...
	if !found {
		return nil, 0, errors.Wrap(niumahome.ErrInternal, "elasticsearch: post_ids aggregation not found")
	}
	// 将聚合结果转换为 map
	...
	for _, bucket := range buckets.([]any) {
		...
		if !found {
			return nil, 0, errors.Wrap(niumahome.ErrInternal, "elasticsearch: key not found in post_ids bucket")
		}
		postID := key.(float64)
	...
	if start != 0 && int(start) >= len(postIDs) { // 错误的分页参数（注意排除搜索结果为空的情况）
		return nil, 0, errors.Wrap(niumahome.ErrInvalidParam, "elasticsearch: wrong paging parameters")
	}
...
func UpdatePost(doc *models.PostDoc) error {
	ctx := context.Background()
	_, err := clnt.Update("niumahome_post_index", strconv.FormatInt(doc.PostID, 10)).Doc(doc).Do(ctx)
	return errors.Wrap(err, "elasticsearch: update failed")
}

func DeletePost(postID int64) error {
	ctx := context.Background()
	resp, err := clnt.Delete("niumahome_post_index", strconv.FormatInt(postID, 10)).Do(ctx)
	if resp.Result.Name == "not_found" {
		return errors.Wrap(niumahome.ErrInternal, "elasticsearch: no such post")
	}
